...
export default function InvoicePage() {
    ...
    async (isPolling = false) => {
      try {
        ...
        if (response.data?.status) {
          ...
          // Status akan diperbarui oleh socket saat pembayaran berhasil
        }
      } catch (err) {
  ...
  // Socket connection untuk pembaruan real-time
  useEffect(() => {
    if (billData?.member_id) {
      socketRef.current = io("https://localhost:3443");
      socketRef.current.emit("register", billData.member_id);
      socketRef.current.on("payment_success", (data) => {
        const hasMatch = data.bill_item_ids.some((id) =>
          billItemIds.includes(id),
        );
        if (hasMatch) {
  ...
  }, [billData?.member_id, billItemIds]);
  ...
  const handlePay = async () => {
    console.log("handlePay called with billItemIds:", billItemIds);
    if (!window.snap) {
    ...
    try {
      console.log("Calling createMidtransTransaction with:", {
      ...
      if (response.data?.status) {
        console.log("Pantau perubahan status di background via socket");

        console.log(
          "Calling window.snap.pay with token:",
          response.data.data.snapToken,
        );
        ...
        console.error("API response status false:", response.data);
        setIsProcessing(false);
      }
    } catch (err) {
            ...
            {!isPaid ? (
              ...
              >
                ...
                <div className="fw-bold">
                  Transaksi ini telah dibayar lunas.
                </div>
                <small>
