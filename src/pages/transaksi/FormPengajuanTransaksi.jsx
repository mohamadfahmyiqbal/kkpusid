...
function FormPengajuanTransaksi() {
  ...
  // New Fields for Disbursement and Recipient Validation
  const [namaNasabah, setNamaNasabah] = useState("");
  const [metodePencairan, setMetodePencairan] = useState("Non Tunai");
  const [noRekening, setNoRekening] = useState("");
  const [bankTujuan, setBankTujuan] = useState("");
  const [lokasiPencairan, setLokasiPencairan] = useState("");
  const [tanggalPencairan, setTanggalPencairan] = useState("");
  const [jamPencairan, setJamPencairan] = useState("");

  // Populate Data from User Profile
  useEffect(() => {
    if (userData) {
      setNamaNasabah((prev) => prev || userData.full_name || userData.name || "");
      if (userData.bank_info) {
        setBankTujuan((prev) => prev || userData.bank_info.bank_name || "");
        setNoRekening((prev) => prev || userData.bank_info.bank_account_no || "");
      }
    }
  }, [userData]);
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        ...
        const response = await UTransaksi.getOptions();
        if (response.data?.status) {
          setCategories(response.data.data.categories || []);
          setTerms(response.data.data.terms || []);
          if (response.data.data.categories.length > 0) {
            setTipeDipilih(response.data.data.categories[0]);
          }
        }
      } catch (err) {
        console.error(err);
  ...
  const localValidationError = useMemo(() => {
    ...
    if (metodePencairan === "Non Tunai") {
      if (!noRekening.trim()) {
        return "Nomor rekening wajib diisi.";
      }
      if (!bankTujuan.trim()) {
        return "Bank tujuan wajib diisi.";
      }
      if (!/^\d{10,}$/.test(noRekening)) {
        return "Nomor rekening minimal 10 digit dan hanya boleh angka.";
      }
    }
    if (metodePencairan === "Tunai") {
      if (!lokasiPencairan.trim()) {
        return "Lokasi pencairan wajib diisi.";
      }
      if (!tanggalPencairan) {
        return "Tanggal pencairan wajib diisi.";
      }
      if (!jamPencairan) {
        return "Jam pencairan wajib diisi.";
      }
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      const selectedDate = new Date(tanggalPencairan);
      if (selectedDate < today) {
        return "Tanggal pencairan tidak boleh kurang dari hari ini.";
      }
    }
    return null;
  }, [
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      if (!namaNasabah.trim() || namaNasabah.trim().length < 3) {
        setValidationError("Nama penerima minimal 3 karakter.");
        return;
      }
      if (metodePencairan === "Non Tunai") {
        if (!noRekening.trim() || !bankTujuan.trim() || !/^\d{10,}$/.test(noRekening)) {
          setValidationError("Data nomor rekening atau bank tujuan tidak valid.");
          return;
        }
      } else if (metodePencairan === "Tunai") {
        if (!lokasiPencairan.trim() || !tanggalPencairan || !jamPencairan) {
          setValidationError("Data pencairan tunai wajib dilengkapi.");
          return;
        }
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const selectedDate = new Date(tanggalPencairan);
        if (selectedDate < today) {
          setValidationError("Tanggal pencairan tidak boleh kurang dari hari ini.");
          return;
        }
      }
      ...
      try {
        ...
        if (metodePencairan === "Non Tunai") {
          payload.no_rekening = noRekening;
          payload.bank_tujuan = bankTujuan.trim();
        } else {
          payload.lokasi_pencairan = lokasiPencairan.trim();
          payload.tanggal_pencairan = tanggalPencairan;
          payload.jam_pencairan = jamPencairan;
        }
              ...
              <Card className="border-0 shadow-sm rounded-4 overflow-hidden custom-card">
                <Card.Body className="p-4">
                  ...
                  <Form onSubmit={handleSubmit}>
                    ...
                    {/* SECTION 2: METODE PENCAIRAN & PENERIMA */}
                    <div className="border-top pt-4 mt-4 mb-3">
                      <h6 className="fw-bold text-slate-800 mb-1 d-flex align-items-center gap-2">
                        <MdShield className="text-primary" size={18} /> Metode Pencairan & Penerima
                      </h6>
                      <span className="text-muted small">Tentukan rekening tujuan atau metode penyerahan dana pembiayaan</span>
                    </div>

                    {/* Nama Penerima / Nasabah */}
                    <Form.Group className="mb-4">
                      <Form.Label className="fw-bold mb-1.5 d-flex align-items-center gap-1.5 small text-muted">
                        <MdPerson size={16} /> Nama Penerima (Sesuai Identitas)
                      </Form.Label>
                      <InputGroup className="custom-input-group shadow-sm">
                        <Form.Control
                          type="text"
                    ...
                    <Form.Group className="mb-4">
                      <Form.Label className="fw-bold mb-2 small text-muted">
                        Metode Penerimaan Dana
                      </Form.Label>
                      <div className="d-flex gap-2">
                        <button
                          type="button"
                          onClick={() => setMetodePencairan("Non Tunai")}
                          className={`flex-grow-1 py-2.5 rounded-3 border fw-bold d-flex align-items-center justify-content-center gap-2 btn-metode ${
                            metodePencairan === "Non Tunai" ? "active" : ""
                          }`}
                        >
                    ...
                    {/* Bidang Input Kondisional */}
                    {metodePencairan === "Non Tunai" ? (
                      <>
                        {/* Bank Tujuan */}
                        <Form.Group className="mb-3">
                          <Form.Label className="fw-bold mb-1.5 d-flex align-items-center gap-1.5 small text-muted">
                            <MdAccountBalance size={16} /> Bank Tujuan
                          </Form.Label>
                          <InputGroup className="custom-input-group shadow-sm">
                            <Form.Control
                              type="text"
                        ...
                        <Form.Group className="mb-3">
                          <Form.Label className="fw-bold mb-1.5 d-flex align-items-center gap-1.5 small text-muted">
                            <MdPayment size={16} /> Nomor Rekening
                          </Form.Label>
                          <InputGroup className="custom-input-group shadow-sm">
                            <Form.Control
                              type="text"
                              className="fw-bold text-slate-800"
                              placeholder="Hanya angka, minimal 10 digit"
                              value={noRekening}
                      ...
                      <>
                        ...
                        <Form.Group className="mb-3">
                          <Form.Label className="fw-bold mb-1.5 d-flex align-items-center gap-1.5 small text-muted">
                            <MdLocationOn size={16} /> Lokasi Pencairan
                          </Form.Label>
                          <InputGroup className="custom-input-group shadow-sm">
                            <Form.Control
                              type="text"
                              className="fw-bold text-slate-800"
                              placeholder="Contoh: Kantor Koperasi Pusat, Cabang Malang"
                              value={lokasiPencairan}
                              onChange={(e) => setLokasiPencairan(e.target.value)}
                              required
                            />
                          </InputGroup>
                        </Form.Group>

                        {/* Tanggal & Waktu Pencairan */}
                        <Row className="g-3 mb-3">
                          <Col md={6}>
                            <Form.Group>
                              <Form.Label className="fw-bold mb-1.5 d-flex align-items-center gap-1.5 small text-muted">
                                <MdCalendarToday size={16} /> Tanggal Pencairan
                              </Form.Label>
                              <InputGroup className="custom-input-group shadow-sm">
                                <Form.Control
                                  type="date"
                          ...
                          <Col md={6}>
                            <Form.Group>
                              <Form.Label className="fw-bold mb-1.5 d-flex align-items-center gap-1.5 small text-muted">
                                <MdAccessTime size={16} /> Jam Pencairan
                              </Form.Label>
                              <InputGroup className="custom-input-group shadow-sm">
                                <Form.Control
                                  type="time"
                                  className="fw-bold text-slate-800"
                                  value={jamPencairan}
                                  onChange={(e) => setJamPencairan(e.target.value)}
                                  required
                                />
                              </InputGroup>
                            </Form.Group>
              ...
              <div className="sticky-lg-top sticky-calc-panel">
                <Card className="border-0 shadow-sm rounded-4 overflow-hidden premium-calc-card text-white mb-4">
                  <Card.Body className="p-4 d-flex flex-column h-100">
                    ...
                    <div className="mb-4">
                      <span className="sim-title d-block mb-1">Estimasi Angsuran / Bulan</span>
                    ...
                    <div className="pt-2">
                      ...
                      <div className="sim-detail-row border-top border-white border-opacity-10 pt-3 mt-2">
                        <span className="sim-detail-label">Penerima</span>
                        <span className="sim-detail-val text-truncate" style={{ maxWidth: "160px" }}>{namaNasabah || "-"}</span>
                      </div>
                      <div className="sim-detail-row">
                        <span className="sim-detail-label">Metode</span>
                        <span className="sim-detail-val">{metodePencairan}</span>
                      </div>
                      {metodePencairan === "Non Tunai" && (bankTujuan || noRekening) && (
                        <div className="sim-detail-row">
                          <span className="sim-detail-label">Rekening</span>
                          <span className="sim-detail-val text-truncate" style={{ maxWidth: "160px" }}>
                            {bankTujuan} {noRekening ? `(${noRekening})` : ""}
                          </span>
                        </div>
                      )}
                      {metodePencairan === "Tunai" && lokasiPencairan && (
                        <div className="sim-detail-row">
                          <span className="sim-detail-label">Lokasi</span>
                          <span className="sim-detail-val text-truncate" style={{ maxWidth: "160px" }}>{lokasiPencairan}</span>
                        </div>
                      )}
                    </div>
                  </Card.Body>
